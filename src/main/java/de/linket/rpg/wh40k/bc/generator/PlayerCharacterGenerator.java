package de.linket.rpg.wh40k.bc.generator;
...
public class PlayerCharacterGenerator {
	...
	private void phase4() {
		...
		MotivationType motivation = (MotivationType) this.handleSingleOption(Arrays.asList(MotivationType.values()), "Please select your motiviation: ");
	...
	private List<Characteristic> generateCharacteristics() {
		...
		while (!accepted) {
			try {
				values = Arrays.asList(CharacteristicType.values()).stream().map(item -> new Characteristic(item, this.pc.getRace())).peek(Characteristic::roll).peek(i -> System.out.println("\t" + i))
